...
class Send{
  ...
  static Future<List<Send>> getSendsForClimb(int climbid) async{
    var db = await PitcherDatabase().database;
    var queryRes = await db.rawQuery("SELECT * FROM $tableName s INNER JOIN ${climber.tableName} c ON c._id = s.climberid WHERE climbid = ?;", [climbid]);
    if (queryRes.isEmpty){
      return <Send>[];
    }
    var list = List<Send>.generate(queryRes.length, (index) {
      ...
      return Send(
          sender: sender,
          tries: int.parse(item["tries"].toString())
      );
    });
    return list;
